...
void init_frustum_planes(float fov_x, float fov_y, float z_near, float z_far)
{
    ...
    frustum_planes[LEFT_FRUSTUM_PLANE].point = vec3_new(0, 0, 0); // izquierda
    ...
    frustum_planes[RIGHT_FRUSTUM_PLANE].point = vec3_new(0, 0, 0); // derecha
    ...
    frustum_planes[TOP_FRUSTUM_PLANE].point = vec3_new(0, 0, 0); // arriba
    frustum_planes[TOP_FRUSTUM_PLANE].normal.x = 0;
    frustum_planes[TOP_FRUSTUM_PLANE].normal.y = -cos_half_fov_y;
    frustum_planes[TOP_FRUSTUM_PLANE].normal.z = sin_half_fov_y;

    frustum_planes[BOTTOM_FRUSTUM_PLANE].point = vec3_new(0, 0, 0); // abajo
...
// si t es 0 retorna a
// si t es 1 retorna b
// si es un valor entre 0 y 1 busca el valor entre a y b
...
void clip_polygon_against_plane(polygon_t *polygon, int plane)
{
    ...
    // Continue with the previous with the last polygon vertex and texture coordinate
    ...
    while (current_vertex != &polygon->vertices[polygon->num_vertices])
    {
        ...
        if (current_dot * previous_dot < 0)
        {
            ...
            // Lo mismo que antes usando interpolación lineal
            ...
            // Usamos la interpolación lineal (lerp) para conseguir las coordnadas interpoladas de U y V
            ...
            // Insert the inteprolated texture to the list of "inside texcoords"
        ...
        // Move to the next vertex and texcoord
    ...
    // At the end, copy the list of inside vertices and texcoords into the destination polygon (out parameter)
